...
import utils.compute_zoom as compute
from utils.search import fetch_image_batch
...
def main():
    ...
    location_input = st.text_input('Enter center latitude and longitude (comma-separated)', '9.115965, 39.174287')
    try:
    ...
    if 'search_results' not in st.session_state:
        ...
        st.session_state.new_sqkm = None

    # Layout: Divide the page into two columns
    img_col1, img_col2 = st.columns(2)

    with img_col1:
        # To display a preview of the area to be checked
        if st.button('Preview Search Area'):
            with st.spinner('Fetching image of search area...'):
                # Compute the estimated zoom level for the requested square area
                zoom = compute.zoom_from_sqkm(total_area_km)
                # Round down to nearest ineger
                zoom_rounded = math.floor(zoom)
                # Compute actual square area for preview image
                st.session_state.new_sqkm = compute.sqkm_from_zoom(zoom_rounded)
                # Fetch image
                preview_image = asyncio.run(fetch_image_batch([(center_lat, center_lon)], zoom = zoom_rounded, api_key = api_key))
                st.session_state.preview_image = preview_image

        # Display preview image if it has been run
        if st.session_state.preview_image:
            # st.write(f'Image Area rounded from {total_area_km} to {st.session_state.new_sqkm}')
